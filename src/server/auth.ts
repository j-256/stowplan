...
interface Statement { bind(...values: unknown[]): Statement; first<T>(): Promise<T|null>; run(): Promise<{success:boolean;meta?:{changes?:number}}> }
interface AuthDb extends D1DatabaseLike { prepare(query:string): Statement }
...
export async function issueSession(db:AuthDb,env:RuntimeEnv,user:SessionUser,request:Request,maximumSeconds?:number){const raw=token(),now=nowIso(),configured=Math.max(3600,Math.min(90*86400,Number(env.AUTH_SESSION_TTL_SECONDS??30*86400))),maxAge=maximumSeconds??configured,end=new Date(Date.now()+maxAge*1000).toISOString();await db.prepare("INSERT INTO sessions(session_id,user_id,token_hash,created_at,expires_at,last_seen_at,user_agent,ip_prefix) VALUES(?,?,?,?,?,?,?,?)").bind(newId("ses"),user.userId,await hash(raw),now,end,now,request.headers.get("user-agent")?.slice(0,300)??null,request.headers.get("cf-connecting-ip")?.split(".").slice(0,3).join(".")??null).run();return {raw,maxAge}}
...
export async function createGuestLink(db:AuthDb,workspaceId:string,creator:string,role:"editor"|"viewer",hours=24){const raw=token(),id=newId("guest"),now=nowIso(),end=new Date(Date.now()+Math.max(1,Math.min(168,hours))*3_600_000).toISOString();await db.prepare("INSERT INTO guest_links(guest_link_id,workspace_id,created_by_user_id,token_hash,role,created_at,expires_at) VALUES(?,?,?,?,?,?,?)").bind(id,workspaceId,creator,await hash(raw),role,now,end).run();return{id,raw,expiresAt:end}}
export async function consumeGuestLink(db:AuthDb,env:RuntimeEnv,raw:string,request:Request){
 ...
 const claimed=await db.prepare("UPDATE guest_links SET consumed_at=? WHERE guest_link_id=? AND consumed_at IS NULL AND revoked_at IS NULL").bind(now,row.guest_link_id).run();
 if((claimed.meta?.changes??0)!==1)throw new Error("Guest link is invalid, expired, used, or revoked");
 const userId=newId("usr"),email=`guest+${row.guest_link_id}@stowplan.invalid`;
 await db.prepare("INSERT INTO users(user_id,email,display_name,global_role,status,created_at,updated_at,last_seen_at) VALUES(?,?, 'Guest', 'user', 'active',?,?,?)").bind(userId,email,now,now,now).run();
 await db.prepare("INSERT INTO identities(identity_id,user_id,provider,provider_subject,email,created_at,last_used_at) VALUES(?,?, 'guest',?,?,?,?)").bind(newId("idn"),userId,row.guest_link_id,email,now,now).run();
 await db.prepare("INSERT INTO workspace_members(workspace_id,user_id,role,created_at) VALUES(?,?,?,?)").bind(row.workspace_id,userId,row.role,now).run();
 const maxAge=Math.max(60,Math.min(86400,Math.floor((Date.parse(row.expires_at)-Date.now())/1000))),session=await issueSession(db,env,{userId,email,displayName:"Guest",globalRole:"user",expiresAt:row.expires_at},request,maxAge);
 return{session,workspaceId:row.workspace_id}
}
...
export async function beginOAuth(db:AuthDb,p:OAuthProvider,base:string,returnTo:string){const state=token(),verifier=b64url(bytes(48)),challenge=b64url(new Uint8Array(await crypto.subtle.digest("SHA-256",new TextEncoder().encode(verifier)))),end=new Date(Date.now()+10*60_000).toISOString();await db.prepare("INSERT INTO oauth_states(state_hash,provider,verifier_ciphertext,return_to,created_at,expires_at) VALUES(?,?,?,?,?,?)").bind(await hash(state),p.id,verifier,returnTo,nowIso(),end).run();const u=new URL(p.authorizationUrl);u.searchParams.set("client_id",p.clientId);u.searchParams.set("redirect_uri",`${base}/api/auth/${p.id}/callback`);u.searchParams.set("response_type","code");u.searchParams.set("scope",p.scopes);u.searchParams.set("state",state);u.searchParams.set("code_challenge",challenge);u.searchParams.set("code_challenge_method","S256");return u.toString()}
