...
| `STOWPLAN_ACCESS_EMAILS` | Access automation | operator shell only | Comma-separated identities merged into the Access Allow policy |
...
`cloudflare/access.json` and `cloudflare/edge-rules.json` contain portable desired state without account, zone, application, policy, identity-provider, or audience IDs. The reconciliation scripts discover those IDs through authenticated API reads. Keep allowlist addresses in `STOWPLAN_ACCESS_EMAILS` or a private config file, not in the checked-in template.
