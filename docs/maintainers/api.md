...
`POST /api/auth/google/start` requires a trusted same-origin mutation, `Content-Type: application/x-www-form-urlencoded`, and a body no larger than 4 KiB. The body contains `cf-turnstile-response` and an `intent` of `sign-in`, `link`, or `reauthenticate`; `returnTo` remains a bounded query parameter. A sign-in request must contain exactly one `termsAccepted=true` and one `sessionPersistence` of `browser-session` or `persistent`. Link and reauthentication requests reject those sign-in-only fields. Every intent requires a fresh Turnstile result whose expected action is `oauth_start` and whose hostname matches `AUTH_BASE_URL`. The server validates sign-in fields and performs Siteverify before allocating OAuth state, then returns a JSON `authorizationUrl` and installs a narrow browser-binding cookie. The client validates the returned Google origin before navigating.
