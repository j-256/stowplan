...
The confirmation page is safe for link previews and scanners to open. New links bind the offered viewer or editor role into the fragment token, so the page explains the exact permission without making a preview request; changing that role makes the token invalid. The recipient signs in when needed, returns to that page in the same browser tab, and explicitly accepts. Stowplan retains the fragment only in that tab while sign-in is in progress and sends the token to the fixed confirmation endpoint only in the bounded same-origin request body after acceptance. Only that action consumes the link. Legacy links with a token in the path remain accepted, are converted to the fragment form before the browser checks authentication, and show neutral role guidance until acceptance.
