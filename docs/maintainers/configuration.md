...
| `LIVE_RELAY_URL` | Cloudflare live collaboration or multi-process Node | build/server | Exact HTTPS origin of the separately deployed live relay, with no path, query, or fragment; it must be present during the application build for CSP and at runtime for capability and publish routes |
