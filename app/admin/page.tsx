...
export default function AdminPage() {
  ...
  return <main className="admin-page"><header><div><p className="eyebrow">Server-enforced control plane</p><h1>Stowplan administration</h1></div><Link href="/">Back to organizer</Link></header>{error && <div className="admin-error"><strong>{error}</strong><p>The admin panel needs a configured server database plus an authenticated app administrator.</p><Link href="/account?returnTo=/admin">Sign in or inspect server setup</Link><Link href="/docs/">Open the testing guide</Link></div>}{!data && !error && <p>Loading administrative records…</p>}{data && <>
