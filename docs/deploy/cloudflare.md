...
The checked-in relay configuration adds two cost guardrails before a Durable Object request. Each successfully authenticated workspace receives separate connect and publish budgets of 600 operations per minute through `LIVE_WORKSPACE_RATE_LIMITER`; a refusal returns `429` with `Retry-After: 60`. The key is the authenticated workspace ID, not a source IP, so unrelated users behind one NAT do not share a counter. Cloudflare evaluates Rate Limiting bindings independently in each location and allows some excess traffic while counters converge, so this is a coarse abuse boundary rather than an exact bill or global quota. Keep the account-unique namespace ID stable across deployments that should share counters, and change it deliberately when resetting that history. See [Workers Rate Limiting bindings](https://developers.cloudflare.com/workers/runtime-apis/bindings/rate-limit/), verified 2026-09-07.

The same configuration caps every relay invocation at 250 ms CPU and 10 total subrequests. No relay invocation appeared in the inspected seven-day Observability window ending 2026-09-07, so this ceiling is derived from the bounded signature verification and single Durable Object hop rather than a measured multiple. Deploy first to a controlled canary and inspect resource-limit outcomes before relying on it for all workspaces. A ceiling failure is visible in Workers metrics and does not authorize bypassing application authentication or moving state through the relay.

Custom `limits.cpu_ms` and `limits.subrequests` require Workers Standard. A Free-oriented fork must omit the `limits` block and rely on Cloudflare's fixed 10 ms CPU, 50 external-subrequest, and 1,000 internal-service-subrequest ceilings. That fallback loses the source-controlled 250 ms and 10-subrequest boundaries, and no representative Free CPU measurement exists. Disabling live collaboration by removing both application relay variables remains the supported Free-compatible fallback: local writes, the durable outbox, ordinary sync, and lifecycle reconciliation continue, but connected clients lose push hints. The execution limits were verified against [Workers limits](https://developers.cloudflare.com/workers/platform/limits/) and [Wrangler limits configuration](https://developers.cloudflare.com/workers/wrangler/configuration/#limits) on 2026-09-07.
