...
`fly secrets list` shows names, not values. Register `https://YOUR_APP.fly.dev/api/auth/google/callback` or the equivalent GitHub callback before sign-in.
...
Fly Volume snapshots are useful infrastructure recovery points, but still test an application-level SQLite backup. Check `fly volumes list`, `fly volumes snapshots list VOLUME_ID`, and `fly releases` before an upgrade. A code rollback does not automatically roll back the SQLite schema.
