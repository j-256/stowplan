...
| `LIVE_RELAY_URL` | Cloudflare live collaboration or multi-process Node | server; build when cross-origin | Exact HTTPS origin used for relay capability and publish routes, with no path, query, or fragment; Sites uses the application origin with `/v1/*` routed to the relay, while cross-origin deployments also provide it during the application build for CSP |
...
Configure `LIVE_RELAY_URL` and `LIVE_RELAY_SECRET` together or leave both unset. A partial pair fails closed. The relay URL is validated as an exact HTTPS origin, except that loopback HTTP is accepted for isolated development. A Sites deployment uses its exact application origin and routes only `/v1/*` on that hostname to the relay Worker so the WebSocket remains inside Sites' same-origin connection policy. A direct Worker or multi-process Node deployment may use a distinct relay origin, but must also provide that URL to `next.config.ts` during the application build so the CSP includes both its HTTPS and WebSocket origins. Keep the secret stable and identical on the application and relay. A relay outage never changes command acceptance or server authorization: committed snapshots remain authoritative, notification failure is contained, and focus, visibility, online, and manual reconciliation remain available.
