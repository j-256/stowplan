...
const snapshotQuotaOrder = Object.freeze([
...
function nestedArrayCount(value: unknown, collection: string, field: string): number {
  if (!isRecord(value) || !Array.isArray(value[collection])) return 0;
  return value[collection].reduce(
    (total, entry) =>
      total + (isRecord(entry) && Array.isArray(entry[field]) ? entry[field].length : 0),
    0,
  );
}
...
export function snapshotQuotaUsage(value: unknown): SnapshotQuotaUsage {
  return {
    ...
    activityPatchesPerSnapshot: nestedArrayCount(value, "activities", "patches"),
...
export function assertSnapshotWithinQuotas(
  ...
  for (const quota of snapshotQuotaOrder) {
