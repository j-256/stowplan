...
function Onboarding({ currentId, currentName, isDemo = false, online, statusRevision, onContinue, onOpenDemo, onOpenWorkspace, onRemoveWorkspace, onResetDemo, onStart }: {
  ...
  return <main className="onboarding workspace-home"><section><Brand /><div className="workspace-home-heading"><div><p className="eyebrow">Workspaces</p><h1>Where to next?</h1><p>Choose a local workspace and see exactly what has—or has not—reached the server.</p></div><span className="connectivity" data-online={online}>{online ? <Wifi /> : <WifiOff />}{online ? "Online" : "Offline"}</span></div><div className="workspace-cards">{workspaces.map((workspace) => {
    ...
    const localOnly = workspace.lastSyncError === "Server backup is not configured for this deployment.";
...
function Inventory({ state, commit }: { state: WorkspaceState; commit: Commit }) {
  ...
  const inventoryRow = (item: ItemRecord) => {
    ...
    return <div className="inventory-row" data-item-id={item.id} data-drop-target={canReorder ? "item" : undefined} data-drop-id={canReorder ? item.id : undefined} key={item.id} draggable={canReorder} onDragStart={canReorder ? (event) => writeDrag(event, { type: "item", id: item.id }) : undefined} onDragOver={canReorder ? (event) => event.preventDefault() : undefined} onDrop={canReorder ? (event) => dropOnItem(event, item) : undefined}>{canReorder ? <TouchDragHandle label={`Drag ${item.name} to reorder`} onDrop={(target) => moveItemByDrop({ type: "item", id: item.id }, target)} /> : <span className="inventory-marker" aria-hidden>•</span>}<label className="inventory-select"><input aria-label={`Select ${item.name} in ${locationName.get(item.locationId) ?? "Unknown space"}`} type="checkbox" checked={activeSelection.includes(item.id)} onChange={() => setSelected((current) => { const valid = current.filter((id) => shownIds.has(id)); return valid.includes(item.id) ? valid.filter((id) => id !== item.id) : [...valid, item.id]; })} /></label><button className="item-name" aria-label={`Open ${item.name} in ${locationName.get(item.locationId) ?? "Unknown space"}`} onClick={() => setEditing(item.id)}><strong>{item.name}</strong><small>{item.category} · {item.frequency} · {item.tags.join(", ") || "no tags"}</small></button><b>{item.quantity} {item.unit}</b><span className="location-path">{locationName.get(item.locationId)}</span>{canReorder && <span className="inventory-order-actions"><button type="button" className="icon small" aria-label={`Move ${item.name} up`} disabled={siblingIndex === 0} onClick={() => reorderItem(item, -1)}><ArrowUp /></button><button type="button" className="icon small" aria-label={`Move ${item.name} down`} disabled={siblingIndex === siblings.length - 1} onClick={() => reorderItem(item, 1)}><ArrowDown /></button></span>}<button className="row-action" aria-label={`Edit or move ${item.name} in ${locationName.get(item.locationId) ?? "Unknown space"}`} onClick={() => setEditing(item.id)}><Edit3 /><span>Edit / move</span></button></div>;
  };
